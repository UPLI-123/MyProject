...
public class StudentController {
    ...
    @ApiOperation("分页查询报名参加的学生的信息")
    @GetMapping("/students/{page}/{size}/{id}")
    public ApiResult findAll(@PathVariable Integer page, @PathVariable Integer size,@PathVariable Integer id) {
        Page<UserTemp> studentPage = new Page<>(page,size);
        IPage<UserTemp> all = studentService.findAll(studentPage,id);
        return  ApiResultHandler.buildApiResult(200,"分页查询所有学生",all);
    }
