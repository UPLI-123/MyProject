...
<template>
  <div class="add">
    <el-page-header @back="goBack" content="增加试题">
    </el-page-header>
    ...
    <el-tab-pane name="second" v-if="isuse()">
...
export default {
  ...
  methods: {
    ...
    create() {
      ...
      // 创建时，先进行一系列的判断
      if(this.changeNumber == '' || this.changeNumber <= 0){
        this.$message({
          type:'warning',
          message:'选择题数量不符合规范'
        }) ;
      ...
      if(this.judgeNumber == '' || this.judgeNumber <= 0){
        this.$message({
          type:'warning',
          message:'判断题数量不符合规范'
        }) ;
      ...
      if(this.fillNumber == '' || this.fillNumber <= 0){
        this.$message({
          type:'warning',
          message:'填空题数量不符合规范'
        }) ;
    ...
    //     }
    //   }).then(() => {
    //     this.$axios(`/multiQuestionId`).then(res => { //获取当前题目的questionId
    ...
    //     }
    //   }).then(() => {
    //     this.$axios(`/fillQuestionId`).then(res => { //获取当前题目的questionId
    //       let questionId = res.data.questionId
    //       this.postPaper.questionId = questionId
    ...
    //     }
    //   }).then(() => {
    //     this.$axios(`/judgeQuestionId`).then(res => { //获取当前题目的questionId
    //       let questionId = res.data.questionId
    //       this.postPaper.questionId = questionId
